...
cap = cv2.VideoCapture(0)  # Change 0 to file path for video input
...
def train_model(features):
    ...
    clf = OneClassSVM(kernel='rbf', gamma='scale', nu=0.05)  # Anomaly detection
...
# Function to display or save frames (handles both local and Jupyter environments)
def show_or_save_frame(frame, fg_mask, frame_count):
    try:
        ...
        # If in a headless environment (no imshow support), use matplotlib or save images
        ...
        # For Jupyter or headless environments, display using matplotlib
...
while True:
    ...
    if len(features) > 0:
        motion_features.extend(features)

    # Once enough data is collected, train the model (increase to 200 frames for better training)
    ...
    # Simulate frame delay with time.sleep()
    time.sleep(0.03)  # 30ms delay, equivalent to 30 fps

    # Break loop after 300 frames or some other condition
    if frame_count > 300:  # Example: run for 300 frames
        break

# After the video capture loop ends, release resources
cap.release()